...
describe('Testing regions route', function () {
  ...
  it('Testing GET /regions route', async function () {
    ...
    expect(response.body.message).to.be.equal('Regiões obtidas com sucesso');
  });
  ...
  it('Testing GET /regions/:id route', async function () {
    ...
    expect(response.body.message).to.be.equal('Região obtida com sucesso');
  });
  ...
  it('Testing GET /regions/:id route with invalid id', async function () {
    ...
    expect(response.body.message).to.be.equal('Nenhuma região foi encontrada com o id 123');
  });
  ...
  it('Testing POST /regions route', async function () {
    ...
    expect(response.body).to.have.property('data');
    expect(response.body.data).to.be.an('object');
    expect(response.body.message).to.be.equal('Região criada com sucesso');
  });
  ...
  it('Testing POST /regions route with invalid data', async function () {
    ...
    expect(response).to.have.property('body');
    expect(response.body).to.have.property('message');
    expect(response.body.message).to.be.equal('É necessário o campo "coordinates.lat"');
  });
  ...
  it('Testing PUT /regions/:id route', async function () {
    ...
    expect(response).to.have.property('body');
    expect(response.body).to.have.property('message');
    expect(response.body.message).to.be.equal('Região atualizada com sucesso');
  });
  ...
  it('Testing GET /regions/distance route', async function () {
    ...
    expect(response).to.have.property('body');
    expect(response.body).to.have.property('message');
    expect(response.body.message).to.be.equal('Nenhuma região foi encontrada nesse raio');
  });
