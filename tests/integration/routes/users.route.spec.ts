...
describe('Testing users route', function () {
  ...
  it('Testing PUT /users/:id route', async function () {
    ...
    expect(response.body).to.have.property('message');
    expect(response.body.message).to.be.equal('Usuário atualizado com sucesso');
  });
  ...
  it('Testing PUT /users/:id route with invalid id', async function () {
    ...
    expect(response).to.have.property('status', 404);
    expect(response.body).to.have.property('message');
    expect(response.body.message).to.be.equal('Nenhum usuário foi encontrado com o id 123123');
  });
